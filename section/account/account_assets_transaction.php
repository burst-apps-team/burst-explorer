...
$nr = count_account_asset_transfers;
$pages = ($_GET['page'])?(int)$_GET['page']:0;
$asset_holders = get_account_asset_transfer($account_id,$pages);
echo show_pages($pages,'account&account='.$_GET['account'].'&submenu=aat',$nr);
?>
...
?>
	<tr>
		...
		<td>
			<a href="?action=account&account=<?=toUnsignedLong($ah['sender_id'])?>"><?=show_account_id_name_and_or_rs($ah['sender_id'],6)?></a>
			<br><span class=" small"><?=show_account_id_name_and_or_rs($ah['sender_id'],1)?></span>
		</td>
		<td>
			<?=asset_quantity($ah['quantity'],$asset_info['decimals'])?>
		</td>
		<td>
			<a href="?action=account&account=<?=toUnsignedLong($ah['recipient_id'])?>"><?=show_account_id_name_and_or_rs($ah['recipient_id'],6)?></a>
			<br><span class=" small"><?=show_account_id_name_and_or_rs($ah['recipient_id'],1)?></span>
		</td>
	</tr>
<?}?>
</table>
